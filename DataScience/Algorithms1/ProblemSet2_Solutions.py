...
class TopKHeap:
    ...
    def insert(self, elt):
        ...
        # your code here
        index_to_find = self.k - 1
        print(index_to_find)
        last_in_k = self.get_jth_element(index_to_find);
        print (last_in_k)
        if elt < last_in_k:
            print("size before = ", self.size())
            self.A.pop(index_to_find)
            print("size after = ", self.size())
            self.insert_into_A(elt)
            self.H.insert(last_in_k)
            return;
        else :
            self.H.insert(elt)
            return;
    ...
    def delete_top_k(self, j):
        ...
        assert j < self.k
        # your code here
        print("deleting = ",  self.A[j])
        j1 = j
        while (j1 < self.k -1):
            print("j1 = ",  j1)
            print("A[j1]= ",  self.A[j1])
            # Swap A[j] and A[j+1]
            self.A[j1] = self.A[j1+1]
            j1 = j1 + 1
        self.A[k-1] = self.H.min_element()
        self.H.delete_min()
        return;
...
class MedianMaintainingHeap:
    ...
    def get_median(self):
        ...
        if hmin_size == hmax_size:
            median_element = (self.hmin.min_element() + self.hmax.max_element()) /2
    ...
    def balance_heap_sizes(self):
        ...
        hmin_size = self.hmin.size()
        hmax_size = self.hmax.size()
        print('hmin_size ', hmin_size)
        print('hmax_size ', hmax_size)
        if hmin_size > hmax_size:
            min_min = self.hmin.min_element()
            self.hmin.delete_min()
            self.hmax.insert(min_min)
        else:
            max_max = self.hmax.max_element()
            print('max_max ', max_max)
            self.hmax.delete_max()
            self.hmin.insert(max_max)
    ...
    def insert(self, elt):
        ...
        hmin_size = self.hmin.size()
        hmax_size = self.hmax.size()
        if hmin_size > hmax_size + 1 or hmax_size > hmin_size + 1 :
